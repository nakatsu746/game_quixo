...
def choice_com(loop):
    ...
    print("{} {} {} {} {}".format(win_simulation[0],win_simulation[1],win_simulation[2],win_simulation[3],win_simulation[4]))
    print("{} {} {} {} {}".format(win_simulation[5],win_simulation[6],win_simulation[7],win_simulation[8],win_simulation[9]))
    print("{} {} {} {} {}".format(win_simulation[10],win_simulation[11],win_simulation[12],win_simulation[13],win_simulation[14]))
    print("{} {} {} {} {}".format(win_simulation[15],win_simulation[16],win_simulation[17],win_simulation[18],win_simulation[19]))
    print("{} {} {} {} {}".format(win_simulation[20],win_simulation[21],win_simulation[22],win_simulation[23],win_simulation[24]))
...
def main():
    ...
    while True:
        ...
        if idx == 0:
            ...
            # 図：円形の黒いボード
            pygame.draw.circle(screen, BLACK, [SCREEN_SIZE/2, SCREEN_SIZE/2], SCREEN_SIZE/2 - 100)

            # 先攻／後攻の選択            
            # 画像：「X」の駒(先攻)
            img_batsu = pygame.transform.rotozoom(img_piece[BATSU], 0, 1.5)
            draw_text(screen, "FIRST ATTACK", SCREEN_SIZE/2 - 170, SCREEN_SIZE/2 + 120, 40, WHITE)
            if ((SCREEN_SIZE/2 - 170) - img_batsu.get_width()/2 <= mouseX  <= (SCREEN_SIZE/2 - 170) + img_batsu.get_width()/2) and \
               (SCREEN_SIZE/2 - img_batsu.get_height()/2 <= mouseY <= SCREEN_SIZE/2 + img_batsu.get_height()/2):
                if tmr%30 < 20:
                    screen.blit(img_batsu, [(SCREEN_SIZE/2 - 170) - img_batsu.get_width()/2, SCREEN_SIZE/2 - img_batsu.get_height()/2])
                if mb == True: #「X」を選択
                    player = 0
                    computer = 1
                    idx = 1
            ...
            draw_text(screen, "SECOND ATTACK", SCREEN_SIZE/2 + 170, SCREEN_SIZE/2 + 120, 40, WHITE)
            if ((SCREEN_SIZE/2 + 170) - img_maru.get_width()/2 <= mouseX <= (SCREEN_SIZE/2 + 170) + img_maru.get_width()/2) and \
               (SCREEN_SIZE/2 - img_maru.get_height()/2 <= mouseY <= SCREEN_SIZE/2 + img_maru.get_height()/2):
                if tmr%30 < 20:
                    screen.blit(img_maru, [(SCREEN_SIZE/2 + 170) - img_maru.get_width()/2, SCREEN_SIZE/2 - img_maru.get_height()/2])
                if mb == True: #「○」の選択
                    player = 1
                    computer = 0
                    idx = 1
        ...
        # 移動させる駒を選ぶ
            ...
            if tmr > 10:
                ...
                if select_piece == True:
                    ...
                    tmr = 0
                    print("choice")
                    print("y:{} x:{}".format(choice_y, choice_x))
